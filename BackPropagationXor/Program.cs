...
namespace BackPropagationXor
{
    class Program
    {
        ...
        class Neuron
        {
            ...
            {
                weights[0] -= LEARN_RATE * error * inputs[0];
                weights[1] -= LEARN_RATE * error * inputs[1];
        ...
        private static void train()
        {
            ...
            for (int i = 0; i < 4; i++)  // very important, do NOT train for only one example
            {
                ...
                Console.WriteLine("{0} xor {1} = {2} ({3}), Correct: {4}", inputs[i, 0], inputs[i, 1], outputNeuron.output > 0.5 ? 1 : 0, outputNeuron.output, ((int)inputs[i, 0] ^ (int)inputs[i, 1]));

                // 2) back propagation (adjusts weights)

                // adjusts the weight of the output neuron, based on its error
                outputNeuron.error =
                    sigmoid.derivative(outputNeuron.output) * -(results[i] - outputNeuron.output);
